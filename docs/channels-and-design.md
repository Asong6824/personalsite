...
- 首屏直接提供四个频道入口及文章数量，下方展示最近 12 篇文章；信息层级以快速扫读和直接到达为优先。
- 页面保持 Server Component，避免为纯导航与文章目录引入额外客户端运行时。
...
- **文章详情页**：采用「顶部信息区 → 独立媒体区 → 正文阅读区」三段结构。桌面端以 `75vw` 容器和 12 栅格组织：标题/摘要占中间 6 栏，右侧基础信息占 2 栏并与标题内容块底部对齐；媒体区占中间 10 栏；正文区占中间 8 栏，目录占右侧 2 栏并与正文保持 1 栏呼吸空间。目录仅在正文区右侧展示，只展示正文 H2-H4，不重复文章标题。
