...
function buildRangeDays(daysCount: any) {
...
const SYN = {
  ...
  kr_weight: ['weight', '权重', '比例', 'weight_pct'],
};

function findPropByType(props: any, type: any) {
...
function findPropByName(props: any, names: any) {
...
function findNumberBySyn(props: any, names: any) {
  const set = new Set((names || []).map((n: any) => n.toLowerCase()));
...
function detectActivityProps(detail: any) {
...
function detectGoalProps(detail: any) {
  const props = detail.properties || {};
...
function detectKrProps(detail: any) {
  const props = detail.properties || {};
...
async function dsQueryAll(client: any, data_source_id: any) {
  let results = [];
...
function qualityToFactor(q: any) {
  const num = Number.isFinite(q) ? q : 1;
  const clamped = Math.max(0, Math.min(5, num));
  return clamped / 5;
}

function normalizeCapToFactor(cap: any) {
  if (!Number.isFinite(cap)) return 1;
  if (cap <= 1) return Math.max(0, cap);
  return Math.min(1, cap / 5);
}

async function queryAllByDate(client: any, database_id: any, dateProp: any, useCreatedTime: any, startISO: any, endISO: any) {
  let results = [];
  let cursor = undefined;
  do {
